module model
    ...
    function run_step(p_active::Float32, p_gf::Float32, p_gj::Float32, Nnew_users::Int64, users::Dict{Int64, Vector{Int64}}, groups::Dict{Int64, Vector{Int64}}, network::Dict{Int64,Set{Int64}}, generate_sample::String, sample_prec::Float32, random_linking::String)
            ...
            users, groups, network = update_dict!(Vold, Vold_groups, groups, users, network, generate_sample, sample_prec)
            ...
            #update bipartite and affiliation networks
            users, groups, network = update_dict!(Vnew, Vnew_groups, groups, users, network, generate_sample, sample_prec)
    ...
    function run_growing_model_ts(p_active::Float32, p_gf::Float32, p_gj::Float32, ts::Vector{Int64}, generate_sample::String, sample_prec::Float32, fname::String, random_linking::String)
        ...
        for tstp in 1:length(ts)
            #println(tstp)
            Nnew_users = ts[tstp]
            users, groups, network = run_step(p_active, p_gf, p_gj, Nnew_users, users, groups, network, generate_sample, sample_prec, random_linking)
            step+=1
            write_in_h5py(fname, step, groups, users)
        end 
    end
